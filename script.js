...
function validatePersonalInfo(event) {
    ...
    if (nameInput.value === null || nameInput.value.trim() === '') {
        ...
        document.getElementById("nameErrM").innerHTML = "This field is required";
    ...
    const phoneRegex = /\+\d{1} \d{3} \d{3} \d{3}/;
    ...
    if (emailRegex.test(emailInput.value) && phoneRegex.test(phoneInput.value)) {
        ...
        if (!emailRegex.test(emailInput.value)) {
            if (emailInput.value === null || emailInput.value.trim() === '') {
                document.getElementById("emailErrM").innerHTML = "This field is required";
            } else {
                document.getElementById("emailErrM").innerHTML = "Invalid Email";
            }
        ...
        if (!phoneRegex.test(phoneInput.value)) {
            if (phoneInput.value === null || phoneInput.value.trim() === '') {
                document.getElementById("phNoErrM").innerHTML = "This field is required";
            } else {
                document.getElementById("phNoErrM").innerHTML = "Invalid Phone Number";
            }
...
for (var i = 0; i < switches.length; i++) {
    switches[i].addEventListener('click', function (event) {
        ...
        if (this.classList.contains('monthly')) {
            ...
            document.querySelector('.arcadeYPrice').innerHTML = "$90/yr";
            document.querySelector('.advancedYPrice').innerHTML = "$120/yr";
            document.querySelector('.proYPrice').innerHTML = "$150/yr";
...
var chosenPlanDict = { "Plan": "Arcade", "Price": 9, "MorY": "Monthly" }; // Default values

// Run selectPlan when next step button is clicked
function selectPlan() {
    if (arcadeClicked) {
        if (MorY === "monthly") {
            chosenPlanDict["Plan"] = "Arcade";
            chosenPlanDict["Price"] = 9;
            chosenPlanDict["MorY"] = "Monthly";
        }

        if (MorY === "yearly") {
            chosenPlanDict["Plan"] = "Arcade";
            chosenPlanDict["Price"] = 90;
            chosenPlanDict["MorY"] = "Yearly";
        }
    }
    ...
    if (advancedClicked) {
        if (MorY === "monthly") {
            ...
            chosenPlanDict["Price"] = 12;
            chosenPlanDict["MorY"] = "Monthly";
        }

        if (MorY === "yearly") {
            chosenPlanDict["Plan"] = "Advanced";
            chosenPlanDict["Price"] = 120;
            chosenPlanDict["MorY"] = "Yearly";
        }
    }
    ...
    if (proClicked) {
        if (MorY === "monthly") {
            ...
            chosenPlanDict["Price"] = 15;
            chosenPlanDict["MorY"] = "Monthly";
        }

        if (MorY === "yearly") {
            chosenPlanDict["Plan"] = "Pro";
            chosenPlanDict["Price"] = 150;
            chosenPlanDict["MorY"] = "Yearly";
        }
    }
...
function MorYPrice() {
    if (chosenPlanDict["MorY"] === "Monthly") {
        document.querySelector('.OnlineServicePrice').innerHTML = "+$1/mo";
        document.querySelector('.largerStoragePrice').innerHTML = "+$2/mo";
        document.querySelector('.customizableProfilePrice').innerHTML = "+$2/mo";
    }
    if (chosenPlanDict["MorY"] === "Yearly") {
        document.querySelector('.OnlineServicePrice').innerHTML = "+$10/yr";
        document.querySelector('.largerStoragePrice').innerHTML = "+$20/yr";
        document.querySelector('.customizableProfilePrice').innerHTML = "+$20/yr";
    }
}
...
function addOnsFunc() {
    if (uncheckedErrM()) {
        if (onlineServiceChecked) {
            if (chosenPlanDict["MorY"] === "Monthly") {
                addOnsDict["OnlineService"] = 1;
            }

            if (chosenPlanDict["MorY"] === "Yearly") {
                addOnsDict["OnlineService"] = 10;
            }
        } else {
        ...
        if (largerStorageChecked) {
            if (chosenPlanDict["MorY"] === "Monthly") {
                addOnsDict["LargerStorage"] = 2;
            }

            if (chosenPlanDict["MorY"] === "Yearly") {
                addOnsDict["LargerStorage"] = 20;
            }
        } else {
        ...
        if (customizableProfileChecked) {
            if (chosenPlanDict["MorY"] === "Monthly") {
                addOnsDict["CustomizableProfile"] = 2;
            }

            if (chosenPlanDict["MorY"] === "Yearly") {
                addOnsDict["CustomizableProfile"] = 20;
            }
        } else {
        ...
        if (chosenPlanDict["MorY"] === "Monthly") {
            document.getElementById("planPrice").innerHTML = "$" + chosenPlanDict["Price"] + "/mo";
            document.querySelector('.total_box p').innerHTML = "Total (per month)";
        }

        if (chosenPlanDict["MorY"] === "Yearly") {
            document.getElementById("planPrice").innerHTML = "$" + chosenPlanDict["Price"] + "/yr";
            document.querySelector('.total_box p').innerHTML = "Total (per year)";
        }
        ...
        if (addOnsDict["OnlineService"] !== 0) {
            ...
            summaryAddonName.textContent = "Online service";

            if (chosenPlanDict["MorY"] === "Monthly") {
                summaryAddonPrice.textContent = "+$" + addOnsDict["OnlineService"] + "/mo";
            }

            if (chosenPlanDict["MorY"] === "Yearly") {
                summaryAddonPrice.textContent = "+$" + addOnsDict["OnlineService"] + "/yr";
            }
        ...
        if (addOnsDict["LargerStorage"] !== 0) {
            ...
            if (largerStorageChecked) {
                summaryAddonName.textContent = "Larger storage";

                if (chosenPlanDict["MorY"] === "Monthly") {
                    summaryAddonPrice.textContent = "+$" + addOnsDict["LargerStorage"] + "/mo";
                }

                if (chosenPlanDict["MorY"] === "Yearly") {
                    summaryAddonPrice.textContent = "+$" + addOnsDict["LargerStorage"] + "/yr";
                }
            }
        ...
        if (addOnsDict["CustomizableProfile"] !== 0) {
            ...
            if (customizableProfileChecked) {
                summaryAddonName.textContent = "Customizable profile";

                if (chosenPlanDict["MorY"] === "Monthly") {
                    summaryAddonPrice.textContent = "+$" + addOnsDict["CustomizableProfile"] + "/mo";
                }

                if (chosenPlanDict["MorY"] === "Yearly") {
                    summaryAddonPrice.textContent = "+$" + addOnsDict["CustomizableProfile"] + "/yr";
                }
            }
        ...
        if (chosenPlanDict["MorY"] === "Monthly") {
            document.querySelector('.summary_total').innerHTML = "$" + totalPrice + "/mo"
        }

        if (chosenPlanDict["MorY"] === "Yearly") {
            document.querySelector('.summary_total').innerHTML = "$" + totalPrice + "/yr"
        }
